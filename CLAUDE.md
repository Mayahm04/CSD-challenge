...
**Profitability rate**: ~7.55% in the market-validated pool (was ~14.78% in costs-only universe, diluted by hybrid EIDs with PR=0). Significantly imbalanced but learnable with `scale_pos_weight`.

**Master dataset**: 211,510 rows, 3,173 EIDs, 48 months, 2 PEAKIDs, 71 columns (55 usable features). Generated by Rachid (includes PSM features from sim_monthly).
...
- **`is_sim_only=1` has 0% positive rate** in training data (no PR/C → PROFIT=0). Exclude from training, cautious about predicting.
- Market-validated pool: 204,161 rows; sim-only pool: 7,349 rows
...
- **sim_monthly** is NOT available locally (too large to download). Code must handle `SIM_MONTHLY_AVAILABLE = False` gracefully. However, PSM features ARE populated in `data/master_dataset.parquet` (Rachid ran the pipeline with sim_monthly on his machine).
...
- `notebook/modeling.ipynb` — modeling pipeline with LightGBM two-stage strategy, Optuna optimization
...
11. **Temporal concept drift** — positive rate drops from 10.7% (2020) to 4.9% (2023). Walk-forward CV mandatory. Threshold must be calibrated on recent validation data, not historical average.
12. **sim-only EIDs** — 0% positive in training. Exclude from training set (`is_sim_only=0` filter).
13. **PROFIT is heavily right-skewed** (median=342, max=212K). Use `log1p(PROFIT)` as regressor target.
14. **`profitable_count_3m` is the #1 feature** (r=0.365 with TARGET). PSM features also strong (~0.18-0.19).
